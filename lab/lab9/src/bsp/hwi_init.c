...
OS_SEC_L4_TEXT void OsGicDisableInt(U32 intId)
{
    ...
    uint32_t shift = (intId % GICD_ICENABLER_SIZE);
    volatile uint32_t* addr = ((volatile U32 *)(uintptr_t)(GICD_ICENABLERn + (intId / GICD_ICENABLER_SIZE) * sizeof(U32))) ;
...
OS_SEC_L4_TEXT void OsGicEnableInt(U32 intId)
{
    ...
    uint32_t shift = (intId % GICD_ISENABLER_SIZE);
    volatile uint32_t* addr = ((volatile U32 *)(uintptr_t)(GICD_ISENABLERn + (intId / GICD_ISENABLER_SIZE) * sizeof(U32))) ;
    uint32_t value = GIC_REG_READ(addr);
    value |= 1 << shift; 
    GIC_REG_WRITE(addr, value);
}
